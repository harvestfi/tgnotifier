...
public class DefaultMessageHandler {
  ...
  private void checkHarvestDto(UserEntity user, HarvestDTO dto, CheckResult checkResult) {
    ...
    if (checkPricePercentChange(user.getLastTvl(), user.getTvlChange(), dto.getLastAllUsdTvl(), dto,
        checkResult)) {
      user.setLastTvl(dto.getLastAllUsdTvl());
      dbService.save(user);
    }
  ...
  private void checkHardWorkDto(UserEntity user, HardWorkDTO dto, CheckResult checkResult) {
    ...
    if (checkPricePercentChange(user.getLastHardWork(), user.getHardWorkChange(), dto.getPsApr(),
        dto,
        checkResult)) {
      user.setLastHardWork(dto.getPsApr());
      dbService.save(user);
  ...
  private void checkCurrentValue(Double userMinAmount, Double dtoAmount, DtoI dto,
  ...
  private boolean checkPricePercentChange(Double userLastValue, Double userChange,
      Double dtoLastValue, DtoI dto,
      CheckResult checkResult) {
    ...
    if (userChange != null &&
      ...
      if (Math.abs(changePercent) > userChange) {
        if (properties.isShowDescriptions()) {
          dto.setDescription(
              "Trigger " + String.format("%.2f%% > %.2f%%", changePercent, userChange));
        }
        checkResult.setSuccess(true);
